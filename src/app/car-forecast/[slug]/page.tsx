...
export function generateStaticParams() {
  return loadStoredCatalog().map((c) => ({ slug: c.slug }));
}
    ...
    <div className="mx-auto max-w-6xl space-y-8 px-6 py-10">
      ...
      <header className="grid gap-6 md:grid-cols-[2fr_1fr] md:items-end">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">
            {car.segment.replace(/-/g, " ")} · {car.bodyStyle}
          </p>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <Stat
          ...
          value={f ? formatPercent(f.cagr5yr, 1) : "—"}
        />
        <Stat
      ...
      {f && !f.insufficientData ? (
        ...
        <Card>
          <CardContent className="py-10 text-center text-amber-300">
            Insufficient auction data — projection withheld.
          </CardContent>
        </Card>
      )}

      <section className="grid gap-6 md:grid-cols-2">
        {car.price ? <ConditionSelector segment={car.segment} baseValueUsd={car.price.valueUsd} /> : null}
